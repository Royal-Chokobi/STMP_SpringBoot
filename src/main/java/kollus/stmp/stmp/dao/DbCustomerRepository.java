...
public interface DbCustomerRepository extends CrudRepository<DbCustomerEntity, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM customer_info WHERE customer_key= :customer_key ", nativeQuery = true)
    Integer deleteCustomerInfo(@Param("customer_key") String customer_key);
